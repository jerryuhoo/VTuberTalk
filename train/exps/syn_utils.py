...
model_alias = {
    ...
    "fastspeech2:StyleFastSpeech2Inference",
    "tacotron2":
    ...
    "istftNet":
    "paddlespeech.t2s.models.istftNet:HiFiGANGenerator",
    "istftNet_inference":
    "paddlespeech.t2s.models.istftNet:HiFiGANInference",
    "unimel":
    "paddlespeech.t2s.models.melgan.universal_melgan:UniversalMelGANGenerator",
    "unimel_inference":
    "paddlespeech.t2s.models.melgan.universal_melgan:UniversalMelGANInference",
    "univnet":
    "paddlespeech.t2s.models.univnet.univnet:UnivnetGenerator",
    "univnet_inference":
    "paddlespeech.t2s.models.univnet.univnet:UnivnetInference",
}
...
def get_am_inference(args, am_config):
    ...
    am_name = args.am[:args.am.index('_')]
    ...
    if am_name == 'fastspeech2':
        if am_dataset in {"aishell3", "vctk"} and args.speaker_dict and not args.use_gst and not args.use_vae and not args.use_style:
            ...
            am_inference = jit.to_static(
                ...
                input_spec=[
                    InputSpec([-1], dtype=paddle.int64), # text
                    InputSpec([1], dtype=paddle.float32), # d_scale
                    InputSpec([1], dtype=paddle.float32), # d_bias
                    InputSpec([1], dtype=paddle.float32), # p_scale
                    InputSpec([1], dtype=paddle.float32), # p_bias
                    InputSpec([1], dtype=paddle.float32), # e_scale
                    InputSpec([1], dtype=paddle.float32), # e_bias
                    InputSpec([1], dtype=paddle.bool), # robot
                    InputSpec([1], dtype=paddle.int64) # spk_id
                ])
        else:
            am_inference = jit.to_static(
